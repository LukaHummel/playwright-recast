...
```bash
pnpm exec playwright test tests/demo.spec.ts --trace on
```
...
```bash
pnpm exec playwright-recast -i ./test-results/demo-dashboard-demo/trace.zip -o demo.mp4
```
...
```bash
pnpm exec tsx scripts/generate-video.ts
```
